...
until the Apple Developer signing identity, notarization credentials, and
release ownership are confirmed.
...
uv run --extra package briefcase package macOS app -p dmg --adhoc-sign --no-notarize --no-input
```
...
Unsigned builds may trigger Gatekeeper warnings. Do not ask John to install an
unsigned build unless that warning is explicitly accepted for the test.
...
2. Confirm notarization credentials are available locally. Prefer a keychain
   profile or app-specific password stored outside the repo.
3. Run the same checks and Briefcase create/build commands.
4. Package with the Developer ID identity rather than the ad-hoc local identity:
...
5. If Briefcase does not automatically sign/notarize from local credentials,
   sign the `.app`, package the DMG, notarize the DMG, staple the ticket, and
   verify with `spctl`.
6. Create a GitHub Release for tag `v<version>` in
   `bldgtyp/bt-web-report-manager`.
7. Upload the DMG with a filename like
   `bt-web-report-manager-<version>.dmg`; the Manager update check prefers the
   first `.dmg` release asset.
8. Install the uploaded DMG on Ed's Mac and confirm Finder launch, update check,
